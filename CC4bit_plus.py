# -*- coding: utf-8 -*-
"""
Created on Thu Sep 10 22:01:28 2026
...
门限筛选：仅 |瞬时值| > K（默认 100000）的通道参与嵌入。
      判定值取"低 4 位清零后的 INT32"，嵌入前后不变，收发两端严格一致。
两种嵌入模式：
  repeat —— 每一帧独立嵌入完整字符串（帧内头: 魔数8b + 字节数8b），
            任意一帧即可解出；过限通道数不足的帧自动跳过（不改）；
...
K_DEFAULT = 100000    # 门限默认值：|瞬时值| > K 的通道才参与嵌入
...
# 第 4 部分：门限判定
# ============================================================
def gated_value(frame: bytes, v_off: int) -> int:
    """门限判定值：低 4 位清零后的 INT32。
    嵌入只改低 4 位，因此该值嵌入前后不变，收发两端门限判定严格一致。"""
    return struct.unpack('>i',
           frame[v_off:v_off+3] + bytes([frame[v_off+3] & 0xF0]))[0]

def channel_gated(frame: bytes, v_off: int, K) -> bool:
    """该通道是否参与嵌入：|门限判定值| > K；K=None 表示不做筛选"""
    return K is None or abs(gated_value(frame, v_off)) > K

# ============================================================
# 第 5 部分：核心 —— INT32 采样值末 4 位嵌入 / 提取（带门限）
# ============================================================
def embed_bits_in_frame(frame: bytes, bits: List[int], pos: int = 0,
                        K=K_DEFAULT) -> Tuple[bytes, int]:
    """
    把 bits[pos:] 依次嵌入帧内各 ASDU 中过门限通道的 seqData 采样值。
    每个 INT32 承载 4 bit：只改该值最低字节的低 4 位，品质字不动。
    返回 (新帧字节, 已消耗比特下标)。
    """
    buf = bytearray(frame)
    for data_off, data_len in find_seqdata_spans(frame):
        for ch in range(data_len // 8):          # 每通道 值4B+品质4B
            v_off = data_off + ch * 8
            if not channel_gated(frame, v_off, K):
                continue                         # 未过门限，跳过
            if pos + 4 > len(bits):
            ...
            pos += 4
            buf[v_off + 3] = (buf[v_off + 3] & 0xF0) | nibble
    return bytes(buf), pos

def extract_bits_from_frame(frame: bytes, K=K_DEFAULT) -> List[int]:
    """从单帧各过门限通道 INT32 采样值低 4 位取出比特流（与发送端同规则）"""
    bits = []
    for data_off, data_len in find_seqdata_spans(frame):
        for ch in range(data_len // 8):
            ...
            if not channel_gated(frame, v_off, K):
                continue
            nibble = frame[v_off + 3] & 0x0F
            bits += [(nibble >> (3 - k)) & 1 for k in range(4)]
    return bits

def frame_capacity_bits(frame: bytes, K=K_DEFAULT) -> int:
    """单帧可承载比特数 = 过门限通道数总和 × 4"""
    n = 0
    for data_off, data_len in find_seqdata_spans(frame):
        for ch in range(data_len // 8):
            if channel_gated(frame, data_off + ch * 8, K):
                n += 4
    return n
...
def embed_message_per_frame(frames: List[bytes], message: bytes,
                            K=K_DEFAULT) -> Tuple[List[bytes], List[int]]:
    """repeat 模式：完整字符串独立嵌入每一帧，任意一帧即可解出。
    帧内格式 = PF_MAGIC(8b) + 报文字节数(8b) + 报文比特。
    过限通道容量不足的帧保持原样并记入 skipped。返回 (新帧列表, 跳过帧下标)"""
    if len(message) > 255:
        raise ValueError("报文超过 255 字节")
    bits = bytes_to_bits(bytes([PF_MAGIC, len(message)])) + bytes_to_bits(message)
    out, skipped = [], []
    for i, f in enumerate(frames):
        if frame_capacity_bits(f, K) < len(bits):
            out.append(f)                        # 容量不足，该帧不嵌入
            skipped.append(i)
            continue
        nf, pos = embed_bits_in_frame(f, bits, 0, K)
        assert pos == len(bits)
        out.append(nf)
    return out, skipped

def extract_message_per_frame(frames: List[bytes], K=K_DEFAULT) -> List:
    """repeat 模式：逐帧独立提取，返回 [(帧序号, 字符串或None)]"""
    res = []
    for i, f in enumerate(frames):
        bits = extract_bits_from_frame(f, K)
...
def embed_message(frames: List[bytes], message: bytes,
                  K=K_DEFAULT) -> List[bytes]:
    """spread 模式：字符串跨帧铺开嵌入一次，流头 32bit = 魔数16b + 字节数16b"""
    bits = bytes_to_bits(struct.pack('>HH', MAGIC, len(message))) \
           + bytes_to_bits(message)
    need = sum(frame_capacity_bits(f, K) for f in frames)
    if len(bits) > need:
        raise ValueError(f"容量不足：需 {len(bits)} bit，目标流仅 {need} bit")
    out, pos = [], 0
    for f in frames:
        nf, pos = embed_bits_in_frame(f, bits, pos, K) if pos < len(bits) else (f, pos)
        out.append(nf)
    return out

def extract_message(frames: List[bytes], K=K_DEFAULT) -> bytes:
...
def embed_pcap_multi(in_pcap: str, out_pcap: str, message: bytes,
                     target_svid: str = None, mode: str = 'repeat',
                     K=K_DEFAULT) -> None:
    """发送端：读 pcap → 分流 → 目标流嵌入（仅过门限通道）→ 按原帧序写出"""
    frames, stamps = read_pcap(in_pcap)
    streams = split_streams(frames)
    print("检测到 SV 流：")
    for (mac, svid), idxs in streams.items():
        print(f"  svID={svid}  源MAC={mac}  帧数={len(idxs)}")
    if target_svid:                            # 指定目标流
        key = next(k for k in streams if k[1] == target_svid)
    else:                                      # 缺省取帧数最多的流
        key = max(streams, key=lambda k: len(streams[k]))
    print(f"目标流：svID={key[1]}，模式：{mode}，门限 K={K}")
    sub = [frames[i] for i in streams[key]]
    if mode == 'repeat':                       # 每帧都嵌入完整字符串
        mod, skipped = embed_message_per_frame(sub, message, K)
        print(f"嵌入完成：{len(sub)-len(skipped)}/{len(sub)} 帧携带"
              + (f"，容量不足跳过帧 {skipped}" if skipped else ""))
    else:                                      # 跨帧铺开，只嵌一次
        mod = embed_message(sub, message, K)
        print("嵌入完成（spread 跨帧铺开）")
    out = list(frames)
...
def extract_pcap_multi(pcap_path: str, mode: str = 'repeat',
                       K=K_DEFAULT) -> dict:
    ...
    for (mac, svid), idxs in split_streams(frames).items():
        ...
        if mode == 'repeat':
            per = extract_message_per_frame(sub, K)
            ok = [x for _, x in per if x is not None]
            results[(mac, svid)] = (ok[0], len(ok), len(per)) if ok else None
        else:
            try:
                results[(mac, svid)] = extract_message(sub, K).decode('utf-8', 'replace')
            except ValueError:
...
if __name__ == '__main__':
    ...
    SECRET   = 'IED42'                   # 要隐藏的字符串
    TARGET   = 'ML1001/LLN0.smvcb0'      # 目标流 svID，None 则自动选最大流
    MODE     = 'repeat'                  # repeat=每帧都嵌入；spread=跨帧嵌一次
    K        = 100000                    # 门限：仅 |瞬时值| > K 的通道参与嵌入

    # 第一步：嵌入
    embed_pcap_multi(IN_PCAP, OUT_PCAP, SECRET.encode('utf-8'), TARGET, MODE, K)

    # 第二步：从输出 pcap 提取验证
    print("\n逐流提取结果：")
    for (mac, svid), r in extract_pcap_multi(OUT_PCAP, MODE, K).items():
        if r:
            msg, ok, total = r
            print(f"  svID={svid} ({mac}): {msg}  —— {ok}/{total} 帧携带")
        else:
